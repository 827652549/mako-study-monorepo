    ...
    () =>
      getStyle(
        ...
        (withCache ? cacheRef.current : [])
          ...
          .reduce((sum, next) => sum + (next - estimatedItemHeight), itemHeightWithMargin * (maxIndex - endIndex)),
      ),
    ...
    mainFrameRef.current = () => {
      ...
      if (
        ...
        (index === 0 &&
          offset <= 0 &&
          marginTopRef.current > 0 &&
          topSpacerRect[propName.top] >= limitsWithOverscanSize[propName.top]) ||
        ...
        marginTopRef.current = 0;
        (ref as {current: {scrollToIndex: Function}})?.current?.scrollToIndex({
          index: 0,
        });
      ...
      if (estimatedItemHeight === 0 || estimatedItemMargin === -1) {
        ...
        const nextItemMargin =
          estimatedItemMargin === -1
            ? Math.ceil(
                (bottomSpacerRect[propName.top] - topSpacerRect[propName.bottom] - itemsHeightSum) / renderedItemsCount,
              )
            : estimatedItemMargin;
      ...
      const bottomSecondElement = bottomElement === topSpacer ? topSpacer : (bottomElement.previousSibling as Element);
      ...
      const isTopBelowTop =
        !isAllAboveTop && !isAllBelowBottom && topSpacerRect[propName.bottom] > limitsWithOverscanSize[propName.top];
      const isBottomAboveBottom =
        !isAllAboveTop && !isAllBelowBottom && bottomSpacerRect[propName.top] < limitsWithOverscanSize[propName.bottom];
